...
def check_user_role(required_role: Union[UserRole, list[UserRole]]):
    async def role_checker(current_user: User = Depends(get_current_user)):
        if isinstance(required_role, list):
            if current_user.role not in required_role and not current_user.is_superuser:
                raise HTTPException(
...
get_admin_user = check_user_role(UserRole.ADMIN)
